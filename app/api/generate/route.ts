...
function buildPrompt(
  ...
  const geyakenSection = isGeyakenMode
    ? `שדה "counter": תובנה גיאקן — קח את התובנה הרגילה שהיית כותב ל"insight", והפוך אותה על הראש. הגע למסקנה שנשמעת הפוכה לאינטואיציה, שרוב האנשים לא יסכימו איתה בהתחלה — אבל אחרי שניה מרגישה נכונה. לא "הצד השני" אלא מסקנה אחרת לגמרי מאותו סיפור. משפט אחד. דוגמה: אם התובנה הרגילה היא "ישיבות מרובות הורגות פרודוקטיביות" — גיאקן יגיד "אנשים שמרגישים בנוח בישיבות הם הבעיה, לא הישיבות עצמן".`
    : 'שדה "counter": null';
